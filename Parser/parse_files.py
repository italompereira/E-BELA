...
class Dump:
    ...
    def get_file(self, remote_file):  # , entities, relations):
        ...
        if os.path.exists(Data.PATH_DUMP + remote_file.split('/')[-1] + ".ttl"):
            print("File " + remote_file.split('/')[-1] + " parsed.")
            return

        print("Downloading file " + remote_file)
        local_file = Data.PATH_DUMP + remote_file.split('/')[-1]
        ...
        try:
            f = open(Data.PATH_DUMP + remote_file.split('/')[-1] + ".csv", "x", encoding="utf-8")
        except:
            print("File " + remote_file.split('/')[-1] + " parsed.")
            return
    ...
    def __call__(self, *args, **kwargs):
        ...
        lines = pathlib.Path("./Parser/dbpedia_files.txt").read_text().splitlines()
