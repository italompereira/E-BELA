...
def check_quality_of_embeddings(embeddings_file):
    ...
    entities = list(set(entities + relations))
    uri_entities = {}
    ...
    check_top_k_by_sentences(
        ['barack obama','barack obama president','barack obama school','barack obama stepfather', 'back to the future', 'donald trump', 'michele obama']#,'Congressional Black Caucus', 'Delta Air Lines', 'moutpiece', 'periodical literature', 'united states', 'usa',
...
def check_top_k_by_sentences(sentences):
    ...
    for i in range(len(vectors)):
        print(sentences[i])
        get_most_similar_to(vectors[i], 50, embeddings_file)
        print('-----\n')
...
def check_top_k_between_entities(uri_entities, embeddings_file) -> None:
    ...
    id_entities = sum([[idx for idx, value in enumerate(entities) if value == uri_entity] for uri_entity in uri_entities], []) #[entities.index(uri_entity) for uri_entity in uri_entities if uri_entity in entities]
...
def generate_embeddings(embeddings_file):
    ...
    for i in sorted(index_entities_not_in_list, reverse=True):
        del entities[i]
    literals_embeddings, literals_relations = transformer.fit(kg, entities)
...
def get_embeddings(embeddings_file):
    ...
    with open(embeddings_file, encoding='utf8') as embedding_file:
        ...
        for i in range(len(lines)):
            ...
            entities_list.append((line_splitted[0], tf.constant([float(x) for x in line_splitted[1].split(' ')])))
...
def get_most_similar_to(vector, k, embeddings_file):
    ...
    for i in indexes:
        ...
        if entity.startswith("https:") or entity.startswith("http:"):
            ...
            try:
                response = kg.connector.fetch(query)
...
def plot(uri_entities):
    ...
    id_entities = sum([[idx for idx, value in enumerate(entities) if value == uri_entity] for uri_entity in uri_entities], [])
    entities_list = [entities_list[i] for i in id_entities]
    embeddings = []
    colors = []
    labels = []
...
if __name__ == "__main__":
    embeddings_file = 'Embeddings/new_embedding_vectors_universal_2.txt'
    if not os.path.isfile(embeddings_file):
        generate_embeddings(embeddings_file)


    #a = tf.constant([5.0,5.0])
    #b = tf.constant([10.0, 10.0])

    #print(tf.keras.losses.CosineSimilarity(axis=0)(a, b).numpy())
