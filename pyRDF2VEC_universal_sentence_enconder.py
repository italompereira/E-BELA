...
def calc_distance(vector_a, vector_b):
    # tf.keras.losses.cosine_similarity(vector_a, vector_b).numpy()
    #
    # cosine_loss = tf.keras.losses.CosineSimilarity(axis=1)
    # cosine_loss(vector_a, vector_b).numpy()
    return tf.reduce_sum(tf.multiply(vector_a,vector_b)).numpy()
...
def check_quality_of_embeddings(embeddings_file):
    ...
    check_top_k_by_sentences(
        ['barack obama','barack obama president','barack obama school','barack obama stepfather']#,'Congressional Black Caucus', 'Delta Air Lines', 'moutpiece', 'periodical literature', 'united states', 'usa',
...
if __name__ == "__main__":
    embeddings_file = 'backup/embedding_vectors_universal.txt'
